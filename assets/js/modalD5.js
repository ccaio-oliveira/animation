...
openMD5.addEventListener('click', () => {
    t1.to('.modal', {
        ...
        .to('.modal-container5', { right: '2%', opacity: 1, stagger: .2 }, '-=.9')

    const closeWithBg = document.querySelector('.modal-bg5');
    closeWithBg.onclick = () => {
        t1.to('.modal-container5', { right: '-100vw', opacity: 0, stagger: .2 }, '-=.9')
            .to('#modal5', { right: -200 })
...
openMD7.onclick = () => {
    ...
    closeWithBg.onclick = () => {
        t1.to('.modal-container7', { right: '-100vw', opacity: 0, stagger: .2 }, '-=.9')
            .to('#modal7', { right: -200 })
    ...
    nextModal.onclick = () => {
        ...
        t1.to('#modal72', { right: 0, top: 0 })
            ...
            .to('.modal-container72', { right: '2%', opacity: 1, stagger: .2 }, '-=.9')

        const closeBtn72 = document.querySelector('.btnClose72');
        closeBtn72.onclick = () => {
            t1.to('.modal-container72', { right: '-100vw', opacity: 0, stagger: .2 }, '-=.9')
...
openMD8.addEventListener('click', () => {
    ...
    closeWithBg.onclick = () => {
        t1.to('.modal-container8', { right: '-100vw', opacity: 0, stagger: .2 }, '-=.9')
            .to('#modal8', { right: -200 })
